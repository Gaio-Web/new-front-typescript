...
function UnsplashGallery1 ({ data }: { data: ImageData }): JSX.Element {
    ...
    <ImageContext.Provider value={value}>
      <Container>
        {isLoading ? (
          ...
          <>
            <img ref={imgRef} src={response[3].urls.small} alt={data.alt_description} onLoad={handleImageLoad} />
            <Overlay className="unsplashImgOverlay" ref={overlayRef}>
              <OverlayText className="overlayText">
                <h3>Imagens sobre seu negócio</h3>
                <h5 className='overlaySubtitle'>Quem sabe um print de conversa com um cliente satisfeito</h5>
              </OverlayText>
            </Overlay>
          </>
        ) : null}
